...
* Output: main_event_study_rural_acpop.ster
*-------------------------------------------------------------------------------
...
keep unique_small_grid_id monthyear downup_ac_pop av_wind_speed wind_direction count ac_uq_id is_rural rice_area_aclvl_ahigh rice_harvarea_aclvl_ahigh rice_prod_aclvl_ahigh
...
foreach mod of local modlist{
	...
	quietly summarize `dep_var' if downup_ac_pop ==0 & TREAT == 1 & `mod' == 1
	local ymean2 = cond(r(N) == 0, `ymean', r(mean))
	
	quietly summarize `dep_var' if downup_ac_pop ==0 & TREAT == 1 & `mod' == 0
...
estwrite evreg* using "${root}/tex/paper/tables/main_event_study${sample}_rural_acpop.ster", replace
estsave_csv evreg1  evreg2  evreg3  evreg4 using "${root}/tex/paper/tables/main_event_study${sample}_rural_acpop.csv", replace
