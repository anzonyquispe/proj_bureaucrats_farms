...
* Generates DiD table with downup_ac_pop treatment
********************************************************************************
...
* Calculate mean DV for control group (downup_ac_pop==0 & TREAT_abs==1)
summarize countk if downup_ac_pop == 0 & TREAT_abs == 1
local meandv = r(mean)
local meandv_fmt = string(`meandv', "%9.3f")


summarize countk if downup_ac_pop == 0 & TREAT_abs == 1 & rice_prod_aclvl_ahigh == 1
local meandv = r(mean)
...
reghdfejl countk i.downup_ac_pop##i.rice_prod_aclvl_ahigh $controls, ///
...
estwrite eq1 eq2 eq3 eq4 using "${table_farms}/_app_20_did_downwind_hm_${sample}_rural_acpop.ster", replace

display "Estimates saved to: ${table_farms}/_app_20_did_downwind_hm_${sample}_rural_acpop.ster"
