...
* Output: _app_17_5km_fe12_evst_all_rural_acpop.csv
*-------------------------------------------------------------------------------
...
foreach mod of local moderators_list {
    ...
    foreach f of numlist 1/1 {
        ...
        * Compute mean of dep var where untreated
        ...
        quietly summarize `dep_var' if `fcond' & treat == 1 & relative_year_bin <= -1 & moderator == 0
        local ymean2 = r(mean)
        ...
        * Count number of unique ACs in subsample
        ...
        foreach fe of numlist 1/1 {
            ...
            * FE indicators for FE12 specification
...
estsave_csv evreg1 evreg2 evreg3 evreg4 evreg5 using "${root}/tex/paper/tables/_app_17_5km_fe12_evst_all${sample}_rural_acpop.csv", replace
