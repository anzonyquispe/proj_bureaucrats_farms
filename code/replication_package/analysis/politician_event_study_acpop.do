...
* Output: _app_16_polischar_fe12_evst_all_rural_acpop.csv
*-------------------------------------------------------------------------------
...
import delimited "${root}/data_output/intermediate/0_master_merge_data_gen${sample}.csv", ///
    clear varnames(1)
keep unique_small_grid_id year month downup_ac_pop
duplicates drop unique_small_grid_id year month, force
tempfile downup_ac_pop_lookup
save `downup_ac_pop_lookup'
restore
merge m:1 unique_small_grid_id year month using `downup_ac_pop_lookup', ///
...
* Moderator variables
local moderators_list moderator downup_ac_pop rice_area_aclvl_ahigh rice_harvarea_aclvl_ahigh rice_prod_aclvl_ahigh
...
foreach mod of local moderators_list {
    ...
    * Attach all tags before storing the estimate.
...
estsave_csv evreg1 evreg2 evreg3 evreg4 evreg5 using "${root}/tex/paper/tables/_app_16_polischar_fe12_evst_all${sample}_rural${ster_suffix}.csv", replace
