...
* Continuous definitions use the baseline ever-treated group for the footer.
bys unique_small_grid_id: egen treat = max(downup_ac)
quietly summarize countk if treat == 1 & relative_year_bin <= -1
local meandv4 = r(mean)
quietly summarize countk if treat == 1 & relative_year_bin <= -1 & moderator == 1
local meandv4_m = r(mean)
...
estadd scalar ymean2 = `meandv4_m'
estadd scalar acq = `numacs'
est store eq4
...
estadd scalar ymean2 = `meandv5_m'
estadd scalar acq = `numacs'
est store eq5
...
estwrite eq* using "${root}/tex/paper/tables/_app_6_main_did_treat_definition${sample}_rural${ster_suffix}.ster", replace

display "Ster: ${root}/tex/paper/tables/_app_6_main_did_treat_definition${sample}_rural${ster_suffix}.ster"
