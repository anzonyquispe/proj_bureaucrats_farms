#!/usr/bin/env python3
"""Generate the three active protest-design figures from their notebook code."""
...
def main() -> None:
    ...
    states = gpd.read_file(require(shared / "data" / "input" / "admin-data" / "india" / "IND_adm" / "IND_adm1.shp"))
    ...
    joined = gpd.GeoDataFrame(joined.merge(grid[["unique_small_grid_id", "geometry"]]), crs=grid.crs)
    ...
    monthly = protests[["year_month", "admin1"]].drop_duplicates().year_month.value_counts().sort_index()
    ...
    ac_id = 285
    events = gpd.sjoin(points, constituencies[["ac_uq_id", "geometry"]], predicate="within")
