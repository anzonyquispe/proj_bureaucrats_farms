#!/usr/bin/env python3
"""Generate the active design maps formerly spread across three notebooks."""
...
def read_inputs(root: Path, shared: Path, sample: str):
    ...
    data = pd.read_csv(
        require(intermediate / f"0_master_merge_data_gen{sample}.csv"),
    ...
    if grid_path is None:
        raise FileNotFoundError("1-grid-generation.shp was not found in either supported intermediate folder")
    ...
    if boundary_path is None:
        raise FileNotFoundError("Constituencies_Boundaries_Post_2008.shp was not found in either supported input tree")
    boundaries = gpd.read_file(boundary_path)
    return intermediate, data, grid, acs, boundaries
...
def map_grids(figures: Path, grid: gpd.GeoDataFrame, acs: gpd.GeoDataFrame,
    ...
    if selected_ac.empty:
        raise ValueError("AC 780, used by the original map, is absent from the AC shapefile")
    ...
    inset.axis("off")
    inset.add_patch(Rectangle((0, 0), 1, 1, transform=inset.transAxes, fill=False, color="red", lw=5))
...
def radius_map(figures: Path, data: pd.DataFrame, grid: gpd.GeoDataFrame,
    ...
    selected = gpd.GeoDataFrame(
        pd.DataFrame({"unique_small_grid_id": ids}).merge(grid), geometry="geometry", crs=grid.crs
    )
...
def time_map(data: pd.DataFrame, grid: gpd.GeoDataFrame, figures: Path,
    ...
    mapped.plot(ax=ax, color=mapped.plot_color, edgecolor="none", linewidth=0, antialiased=False)
    ax.set_title(title)
    ax.axis("off")
...
def sample_region(region, count: int, rng: np.random.Generator) -> np.ndarray:
    if region.is_empty or region.area <= 0:
        raise ValueError("Treatment-panel population target is infeasible for the selected geometry")
...
def treatment_panels(intermediate: Path, shared: Path, figures: Path, data: pd.DataFrame,
                     grid: gpd.GeoDataFrame, acs: gpd.GeoDataFrame) -> None:
    ids = data.loc[
    ...
    if boundary.empty or fire_a.empty or fire_b.empty:
        raise ValueError("Original treatment-panel AC/fire-grid IDs are absent from the current inputs")
    center_a, center_b = fire_a.geometry.centroid.iloc[0], fire_b.geometry.centroid.iloc[0]
    line_a, line_b = make_line(center_a, boundary.total_bounds), make_line(center_b, boundary.total_bounds)
    a_below, b_below = halfplane(line_a, True), halfplane(line_b, True)
    universe = cells.geometry.union_all() if hasattr(cells.geometry, "union_all") else cells.geometry.unary_union
    # The fixed 17-person illustration has 10 below both lines, 3 between
    # the lines, and 4 above both: A=13, B=10, and the complement of B=7.
    ...
    if fire_path is None:
        raise FileNotFoundError("fire.png was not found in the intermediate or proj_downwind figure folders")
    ...
    def plot(line, center, below, flags, filename):
        ...
        shade = cells.geometry.apply(
            lambda geom: geom.intersection(down).area / geom.area > 0.5 if geom.area else False
        )
        ...
        ax.scatter(population[~flags, 0], population[~flags, 1], s=130, facecolor="white",
                   edgecolor="black", linewidth=2.1, zorder=8)
        ...
        ax.add_patch(FancyArrowPatch(tail, head, arrowstyle="-|>", color="blue", lw=4,
                                     mutation_scale=22))
        ax.axis("off")
        fig.savefig(figures / filename, dpi=300, bbox_inches="tight")
        plt.close(fig)
...
def main() -> None:
    ...
    intermediate, data, grid, acs, boundaries = read_inputs(options.root, shared, options.sample)
