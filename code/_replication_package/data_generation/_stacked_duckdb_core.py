#!/usr/bin/env python3
"""Reusable DuckDB engine for stacked DiD datasets around 0 -> 1 switches.
...
Default behavior mirrors the R script:
  ...
  * treatment history: caller-supplied binary column
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build one stacked DiD CSV around a binary treatment's 0->1 "
            "events using disk-backed DuckDB."
        ),
        ...
        if col == args.time_col:
            if args.derive_time:
                select_exprs.append(
                    ...
                    f"TRY_CAST({qid(args.month_col)} AS BIGINT)) AS {qid(col)}"
                )
            else:
            ...
            select_exprs.append(
                "CASE "
                f"WHEN {qid(col)} IS NULL THEN NULL "
                f"WHEN TRY_CAST({qid(col)} AS DOUBLE) = 0.0 THEN 0::TINYINT "
                f"WHEN TRY_CAST({qid(col)} AS DOUBLE) = 1.0 THEN 1::TINYINT "
                f"ELSE 127::TINYINT END AS {qid(col)}"
            )
        else:
    ...
    try:
        ...
        con.execute(
            ...
            SELECT
                ...
                1::INTEGER AS treat,
                CASE
                    WHEN p.{time} >= {int(cohort)} THEN 1::TINYINT
                    ELSE 0::TINYINT
                END AS post,
...
def validate_final_stack(
    ...
    """Check generated indicators and the stacked panel key before export."""
    ...
    (
        ...
        invalid_control_treatment,
    ) = con.execute(
        f"""
        SELECT
            count_if(treat IS NULL OR treat NOT IN (0, 1)),
            count_if(
                post IS NULL
                OR post NOT IN (0, 1)
                OR post <> CASE WHEN {time} >= cohort THEN 1 ELSE 0 END
                OR post <> CASE WHEN relative_monthyear >= 0 THEN 1 ELSE 0 END
            ),
            count_if(relative_monthyear <> {time} - cohort),
            count_if(treat = 0 AND {treatment} IS DISTINCT FROM 0)
        FROM final_stack
        """
    ).fetchone()
    if any(
        ...
        raise ValueError(
            "Final stack validation failed: "
            f"invalid treat={invalid_treat:,}, invalid post={invalid_post:,}, "
            f"invalid relative time={invalid_relative_time:,}, "
    ...
    duplicate_keys, mixed_groups = con.execute(
        ...
        FROM (
            SELECT
                ...
                cohort,
                count(*) AS row_count,
                count(DISTINCT treat) OVER (
                    PARTITION BY {unit}, cohort
                ) AS treat_values
            FROM final_stack
            GROUP BY {unit}, {time}, cohort, treat
        )
        """
    ).fetchone()
    if int(duplicate_keys) or int(mixed_groups):
        raise ValueError(
            "Final stack key/group validation failed: "
            f"duplicate unit-time-cohort keys={duplicate_keys:,}, "
            f"mixed treatment groups={mixed_groups:,}."
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    temp_directory = args.temp_directory or database_path.parent / "duckdb_temp_stacked"
