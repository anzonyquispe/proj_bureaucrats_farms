...
agregation_result <- function(ev, numPrePeriods = 4, numPostPeriods = 5, M = 1, honest = FALSE, omitted_period = -1,
  ...
  p1 <- ggplot(ev, aes(x = t, y = est)) +
    ...
    theme(
      ...
      panel.grid.major   = element_line(colour = alpha("gray70", 0.35), linetype = "dashed"),
    ...
    annotate("text",
             ...
             size  = 4)
  if (!is.null(ylim_ori)) {
    p1 <- p1 + coord_cartesian(ylim = ylim_ori)
  }
  ...
  if (!is.null(ylim_rot)) {
    p2 <- p2 + coord_cartesian(ylim = ylim_rot)
  }
