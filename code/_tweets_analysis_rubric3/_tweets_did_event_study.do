...
foreach outcome of global outcome_list {
    ...
    capture estimates save "${figures}/r3_`outcome'.ster", replace
    capture graph export   "${figures}/r3_`outcome'.png",   replace width(1200)
    display as result "  -> ${figures}/r3_`outcome'.png"
    local n_ok = `n_ok' + 1
}
