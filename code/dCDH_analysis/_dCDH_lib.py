...
Loads the master panel, applies the same balanced-panel + reset transformation
...
Memory-efficient by design:
  * Lazy scan of the master CSV; rural / time / dpl_ac filters pushed into the scan
...
OUT_DIRS = [
    ...
    Path(os.environ.get(
        "DCDH_LOCAL_OUT",
        "/users/aquisper/proj_bureaucrats_farms/code/dCDH_analysis",
    )),
]
...
def load_panel(treatment: str) -> pl.DataFrame:
    ...
    those are then pushed into the CSV scan as `is_in` filters so the master
    CSV never lands in memory at full size.
    """
    print(f"[load] master CSV: {MASTER_CSV}")
    ...
    df = (
        pl.scan_csv(str(MASTER_CSV), ignore_errors=True)
          .select(keep_cols)
