...
#   * If DOWNLOADS_DIR can't be created (e.g. cluster runs), falls back to
#       <root>/data_output/intermediate/stacked_downup_pop<sample>/
#
# Memory model
# ------------
#   * Only the columns needed downstream are read from the master CSV.
#   * Each cohort's stack is written to its own .dta in a per-run folder and
#     dropped from memory immediately -- nothing is accumulated.
#   * Cohorts are processed in parallel via parallel::mclapply across CORES
#     forked workers (default 10). The parent dt is shared by the OS's
#     copy-on-write fork semantics, so workers don't multiply the resident
#     set. The cohort builder only does read-only filtering on dt -- never `:=`.
#
...
output_dir <- tryCatch({
  ...
  if (dir.exists(primary_output_dir)) primary_output_dir else stop()
}, error = function(e) {
  message("DOWNLOADS_DIR not writable; falling back to: ", fallback_output_dir)
...
process_cohort <- function(c_value) {
  ...
  out_file <- file.path(output_dir, sprintf("cohort_%s.dta", as.character(c_value)))
  ...
  # Write to disk as Stata .dta and release memory
  write_dta(out, out_file)
  n <- nrow(out)
