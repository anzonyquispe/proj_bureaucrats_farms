...
*
* Mirror of _stacked_analysis_balanced.do, but reads combined_dt_pop.dta and
* uses downup_ac_pop wherever the original used downup_ac. Enforces that every
* CONTROL grid be observed in every relative_monthyear in [-6, 6] within its
* cohort. Controls that don't span the full 13-period window are dropped.
* Treated grids are kept regardless of their balance.
*
* Output: stacked_event_study_balanced_pop${sample}_rural.ster / .csv
*-------------------------------------------------------------------------------
...
use "${root}/data_output/intermediate/combined_dt_pop.dta", clear
...
estwrite evreg* using "${root}/tex/paper/tables/stacked_event_study_balanced_pop${sample}_rural.ster", replace
estsave_csv evreg1 evreg2 using "${root}/tex/paper/tables/stacked_event_study_balanced_pop${sample}_rural.csv", replace
