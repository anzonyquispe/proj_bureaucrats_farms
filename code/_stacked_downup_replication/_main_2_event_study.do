...
keep unique_small_grid_id monthyear downup_ac_pop av_wind_speed wind_direction countk ac_uq_id is_rural rice_area_aclvl_ahigh rice_harvarea_aclvl_ahigh rice_prod_aclvl_ahigh cohort
...
foreach mod of local modlist{
	...
	* Run regression with clustering
	...
	local numacs = r(unique)
		quietly summarize `dep_var' if downup_ac_pop ==0 & TREAT == 1 & esample == 1
	local ymean = r(mean)
		quietly summarize `dep_var' if downup_ac_pop ==0 & TREAT == 1 & `mod' == 1 & esample == 1
	local ymean2 = cond(r(N) == 0, `ymean', r(mean))
		quietly summarize `dep_var' if downup_ac_pop ==0 & TREAT == 1 & `mod' == 0 & esample == 1
