...
use  "${int_farms}/combined_dt.dta", clear
...
keep unique_small_grid_id monthyear downup_ac av_wind_speed wind_direction countk ac_uq_id is_rural rice_area_aclvl_ahigh rice_harvarea_aclvl_ahigh rice_prod_aclvl_ahigh
...
foreach mod of local modlist{
	...
	* Run regression with clustering
	...
	gen esample = e(sample)
		...
		unique ac_uq_id if esample == 1
	local numacs = r(unique)
		quietly summarize `dep_var' if downup_ac ==0 & TREAT == 1 & esample == 1
	local ymean = r(mean)
		quietly summarize `dep_var' if downup_ac ==0 & TREAT == 1 & `mod' == 1 & esample == 1
	local ymean2 = cond(r(N) == 0, `ymean', r(mean))
		quietly summarize `dep_var' if downup_ac ==0 & TREAT == 1 & `mod' == 0 & esample == 1
...
estwrite evreg* using "${root}/tex/paper/tables/main_event_study${sample}_rural_area_stacked.ster", replace
estsave_csv evreg1  evreg2  evreg3  evreg4 using "${root}/tex/paper/tables/main_event_study${sample}_rural_stacked.csv", replace
