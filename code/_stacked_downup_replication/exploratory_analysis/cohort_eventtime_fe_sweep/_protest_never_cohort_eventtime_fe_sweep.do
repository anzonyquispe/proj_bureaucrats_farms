********************************************************************************
* Protest FE sweep with cohort-specific relative-year fixed effects.
*
* Input: stacked_data_protest5km.csv
* Controls: never treated only.
* Sample: rural; through August 2022; relative years -8,...,1.
* Both event-study and DiD loops use the same loaded, prepared, restricted data.
********************************************************************************
version 17
clear all
set more off
...
display as text "Input: `input_file' | controls: never | FE list: $fe_list"
...
local required unique_small_grid_id province ac_uq_id count month year ///
    ...
    yeargov treat control_type cohort relative_year_bin
...
assert control_type == 1 if treat == 0
...
egen byte cohort_tag = tag(cohort)
count if cohort_tag
...
foreach mod of local moderators_list {
    ...
    foreach selected_fe of numlist $fe_list {
        ...
        local fespec "`original_fespec' relative_year_bin_aux#cohort"
        local tag : display %02.0f `selected_fe'
        local tag = strtrim("`tag'")
        display as result "PROTEST EVENT FE `selected_fe': `fespec'"
        est clear
        ...
        estadd local controls "never"
        estadd local cohortvar "cohort"
        est store evreg1
        local out "${tables}/protest_never_cohorttime_fe`tag'_event_rural${ster_suffix}_all"
...
foreach selected_fe of numlist $fe_list {
    ...
    local tag : display %02.0f `selected_fe'
    local tag = strtrim("`tag'")
    display as result "PROTEST DID FE `selected_fe': `fespec'"
    est clear
    reghdfejl countk `rhs', absorb(`fespec') vce(cluster ac_elec_yr)
    estadd scalar ymean = `ymean'
    estadd scalar ymean2 = `ymean2'
    ...
    estadd local cohortvar "cohort"
    est store evreg1
    local out "${tables}/protest_never_cohorttime_fe`tag'_did_interaction_rural${ster_suffix}_all"
    estwrite evreg1 using "`out'.ster", replace
    confirm file "`out'.ster"
}
}
