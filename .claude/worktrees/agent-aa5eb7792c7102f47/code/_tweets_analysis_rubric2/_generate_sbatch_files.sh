#!/bin/bash
# _generate_sbatch_files.sh  (rubric_2)
#
# Emit one SGE sbatch file per (prefix, suffix) pair (16 total) into ./sbatch/,
...
# outcomes. Mirrors the structure of code/_replication_rural/ and
# code/_tweets_analysis/.
#
# Plan:
#   prefixes (8): ar ff sf pr sl wp cc ba    (rubric_2's 8 fields)
#   suffixes (2): all own
#   total: 16 sbatch files, 1 core each, runnable concurrently.
#
...
CODE_DIR_REMOTE="/users/aquisper/proj_bureaucrats_farms/code/_tweets_analysis_rubric2"
PROJ_SHELL_REMOTE="/groups/sgulzar/sa_fires/proj_bureaucrats_farms"

# --- Outcome universes per rubric_2 field -----------------------------------
# Numeric levels (0-3) -> prefixed without a separator:    ar0..ar3, pr0..pr3
# String values -> prefixed with underscore separator:     ff_<v>, sf_<v>, etc.
NUMERIC_LEVELS="0 1 2 3"
FRAMING_VALS="provider_hero victim protester_activist beneficiary problem_causer neutral_reference rhetorical"
STANCE_FARMERS_VALS="pro_farmer anti_farmer neutral not_applicable"
STANCE_LAWS_VALS="pro_laws anti_laws neutral not_applicable"
WELFARE_VALS="pm_kisan crop_insurance machinery_distribution msp_procurement debt_relief irrigation_scheme soil_health organic_farming other_farm_policy none"
CREDIT_VALS="true false"
BLAME_VALS="central_govt state_govt opposition middlemen bureaucracy other none"
...
build_list () {
    ...
    local sep="_"
    case "$prefix" in
        ar|pr)  items="$NUMERIC_LEVELS";        sep="" ;;
        ff)     items="$FRAMING_VALS" ;;
        sf)     items="$STANCE_FARMERS_VALS" ;;
        sl)     items="$STANCE_LAWS_VALS" ;;
        wp)     items="$WELFARE_VALS" ;;
        cc)     items="$CREDIT_VALS" ;;
        ba)     items="$BLAME_VALS" ;;
...
emit_sbatch () {
    local prefix="$1" suffix="$2"
    local job="tweets_r2_${prefix}_${suffix}"
...
PREFIXES=(ar ff sf pr sl wp cc ba)
SUFFIXES=(all own)
...
echo
echo "Generated $(ls "${SBATCH_DIR}"/tweets_r2_*.sbatch 2>/dev/null | wc -l | tr -d ' ') sbatch files in sbatch/ + submit_all_jobs.sh"
